...
Camera &Camera::setup(SceneObjectPtr player) {
...
void Camera::update_projection_matrix(float aspect_ratio) {
  ...
  m_proj = glm::perspective(m_fovy_rad, m_aspect_ratio_cache, m_z_near, m_z_far);

  float top = m_z_far * tan(m_fovy_rad / 2);
  float right = top * m_aspect_ratio_cache;

  m_ortho_frustum = {
      {-right, -top, m_z_near}, {right, -top, m_z_near}, {right, top, m_z_near},
      {-right, top, m_z_near},  {-right, -top, m_z_far}, {right, -top, m_z_far},
...
Camera& Camera::set_fovy(float fovy_deg) {
...
Camera& Camera::set_sensitivity(float sensitivity) {
  m_sensitivity = glm::max(0.f, sensitivity);
  return *this;
}

float Camera::get_speed() const { return m_speed; }
Camera &Camera::set_speed(float speed) {
  m_speed = glm::max(0.f, speed);
  return *this;
}

float Camera::get_far_plane() const { return m_z_far; }
Camera &Camera::set_far_plane(float z_far) {
  m_z_far = glm::max(0.f, z_far);
  return *this;
}

float Camera::get_near_plane() const { return m_z_near; }
Camera &Camera::set_near_plane(float z_near) {
