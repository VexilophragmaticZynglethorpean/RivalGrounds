...
BoundingBox::BoundingBox() : min(glm::vec3(0.f)), max(glm::vec3(0.f)) {}
...
Where BoundingBox::where(const glm::vec3 &point) const {
...
BoundingBox &BoundingBox::add_point(const glm::vec3 &point) {
...
std::vector<SimpleVertex> BoundingBox::corners() const {
  return {{min.x, min.y, min.z}, {max.x, min.y, min.z}, {min.x, max.y, min.z},
          {max.x, max.y, min.z}, {min.x, min.y, max.z}, {max.x, min.y, max.z},
          {min.x, max.y, max.z}, {max.x, max.y, max.z}};
}

std::vector<LineIndices> BoundingBox::indices() {
  return {
