...
{
private:
  std::optional<RenderPacketPtr> m_render_packet = std::nullopt;
  RenderPacketPtr& create_render_packet(App& app);
...
public:
  ...
  {
    RenderPacketPtr packet = create_render_packet(app);
    f(packet);
    m_render_packet = std::move(packet);
  }
  std::optional<RenderPacketPtr> get_render_packet();
